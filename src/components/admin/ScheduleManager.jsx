...
const ScheduleManager = () => {
  ...
  const handleDuplicate = async (show) => {
    ...
    newShow.thoiGianBatDau =
      show.thoiGianBatDau.slice(0, 10) +
      "T" +
      calculateEndTime(show.thoiGianBatDau, show.phim.thoiLuong);
    console.log(newShow.thoiGianBatDau);
    newShow.thoiGianBatDau = calculateEndTimeWithDate(
      newShow.thoiGianBatDau,
      70
    );
    console.log(newShow);
  ...
  const checkConflict = (show) => {
    ...
    return showtimes.some((s) => {
      if (s.id === show.id || s.room !== show.room || s.date !== show.date)
        return false;
      const sEndTime = calculateEndTime(s.startTime, s.duration);
      return (
        (show.startTime >= s.startTime && show.startTime < sEndTime) ||
  ...
  const filteredShowtimes = showtimes.filter((show) => {
    // Thêm bước kiểm tra: nếu suất chiếu không có đủ thông tin cần thiết thì loại bỏ luôn
    if (
      !show ||
      !show.thoiGianBatDau ||
      !show.phongChieu ||
    ...
    const matchRoom =
      selectedRoom === "all" || show.phongChieu.maPhongChieu === selectedRoom;
    const matchSearch = show.phim.tenPhim
      .toLowerCase()
  ...
  const groupedByRoom = rooms.reduce((acc, room) => {
    acc[room.maPhongChieu] = filteredShowtimes
      .filter((show) => show.phongChieu.maPhongChieu === room.maPhongChieu)
      .sort((a, b) => a.thoiGianBatDau.localeCompare(b.thoiGianBatDau));
    return acc;
  }, {});

  const totalRevenue = filteredShowtimes.reduce(
    (sum, show) => sum + show.donGiaCoSo * show.phongChieu.soLuongGhe,
    0
  );
            ...
            <div className="col-md-3">
              <div className="card shadow-sm border-primary border-start border-4">
                <div className="card-body">
                  ...
                  <div className="h2 mb-0 text-primary fw-bold">
                ...
                <div className="card-body">
                  ...
                  <div className="h2 mb-0 text-info fw-bold">
                    {filteredShowtimes.reduce(
                      (sum, s) => sum + s.phongChieu.soLuongGhe,
                      0
                    )}
                  </div>
                </div>
              ...
              <div className="card shadow-sm border-warning border-start border-4">
                <div className="card-body">
                  ...
                  <div className="h2 mb-0 text-warning fw-bold">
                    {
                      new Set(
                        filteredShowtimes.map((s) => s.phongChieu.maPhongChieu)
                      ).size
                    }
                  </div>
                </div>
              ...
              <div className="card shadow-sm mb-3">
                <div className="card-body">
                  ...
                  <div className="d-grid gap-2">
                    {getWeekDays().map((day) => {
                      const dayShowtimes = showtimes.filter(
                        (s) => dateOnlyFromIsoString(s.thoiGianBatDau) === day
                      );
                      console.log();
                  ...
                  <div className="list-group">
                    ...
                    {rooms.map((room) => (
                      <button
                        ...
                        className={`list-group-item list-group-item-action ${
                          selectedRoom === room.maPhongChieu ? "active" : ""
                        }`}
                ...
                <div className="card-body">
                  ...
                  <div className="d-flex align-items-center justify-content-between mb-4">
                    <div className="d-flex align-items-center gap-3">
                      ...
                      <h2 className="mb-0 h4">{formatDate(selectedDate)}</h2>
                      <button
                        onClick={() => changeDate(1)}
                        className="btn btn-outline-primary"
                      >
                  ...
                  {rooms.map((room) => {
                    ...
                    if (
                      selectedRoom !== "all" &&
                      selectedRoom !== room.maPhongChieu
                    )
                      return null;
                        ...
                        {roomShowtimes.length === 0 ? (
                          ...
                          <div className="row g-3">
                            {roomShowtimes.map((show, idx) => {
                              ...
                              const beginTime = beginDate.toLocaleTimeString(
                                "vi-VN",
                                      ...
                                      <div className="row g-2 mb-3">
                                        ...
                                        <div className="col-6">
                                          <div className="bg-success bg-opacity-10 rounded p-3">
                                            ...
                                            <small className="text-muted">
                                              {show.phongChieu.soLuongGhe} ghế
                                            </small>
                                          </div>
                                        </div>
                                      </div>
                  ...
                  {filteredShowtimes.length === 0 && (
                    <div className="text-center py-5">
                      ...
                      <p className="text-muted">
                        Hãy thêm suất chiếu mới cho ngày này
                      </p>
                    </div>
                  )}
                </div>
